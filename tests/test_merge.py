...
def test_all_forms():
    label = (
        "32-36, 25-31, 1A-F, 2/1-10, 4 bus 1-30, 43, 44 bus 1, 45/1, "
...
def test_house_number_sequences():
    label = (
        "32, 34, 36, 38, 25, 27, 29, 31, 39, 40, 41, 42, 43, 44, 46, "
        "47, 48, 49, 50, 52, 54"
    )
    house_numbers = housenumparser.merge(label)
    assert isinstance(house_numbers, list)
...
def test_special_characters():
    ...
    """
    label = "1ëâB, 1-11"
    house_numbers = housenumparser.merge(label, on_exc=ReadException.Action.DROP)
    assert isinstance(house_numbers, list)
    assert 1 == len(house_numbers)
    assert "1-11" == str(house_numbers[0])
    ...
    house_numbers = housenumparser.merge(label, on_exc=ReadException.Action.ERROR_MSG)
    assert isinstance(house_numbers, list)
    assert 2 == len(house_numbers)
    assert "1-11" == str(house_numbers[0])
